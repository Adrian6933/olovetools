...
import { ArrowRight, Crop, FileImage, Loader2, Minimize2, Share2, Sticker } from 'lucide-react';
import { sendToTool } from '../../../lib/handoff';

interface NextStepBarProps {
  lang: string;
  t: any;
  /** Genera la imagen ya marcada. Se llama en perezoso, sólo al elegir destino. */
  getResult: () => Promise<{ blob: Blob; name: string } | null>;
}

// Una imagen con marca de agua se suele querer más ligera, recortada al formato
// de la tienda, convertida, o publicada en redes.
const STEPS = [
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'watermark-snap');
    } catch {
      setPending(null);
    }
  };
