...
export const Header: React.FC<HeaderProps> = ({ currentLang, onLanguageChange, onReset, t }) => {
  return (
    <div className="fixed top-0 left-0 right-0 z-[100] pointer-events-none">
      <header className="w-full h-auto md:h-24 py-4 md:py-0 border-b border-white/10 bg-[#020205]/95 backdrop-blur-3xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] pointer-events-auto">
        <div className="max-w-7xl mx-auto h-full px-4 md:px-12 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0">
          {/* min-w-0 all the way down: without it "oLoveTools" + the tool name
              cannot shrink and the pair is clipped on a 375 px phone. */}
          <div className="flex items-center justify-between w-full md:w-auto min-w-0 gap-3 md:gap-12">
            {/* oLoveTools Branding */}
            <a 
            ...
            >
              <div className="w-8 h-8 md:w-9 md:h-9 shrink-0 bg-white/5 rounded-lg md:rounded-xl flex items-center justify-center border border-white/10 group-hover:bg-fuchsia-500/20 group-hover:border-fuchsia-500/50 transition-all">
                <Smile className="w-5 h-5 text-fuchsia-400" />
              </div>
              <span className="text-lg sm:text-xl md:text-2xl font-black text-white tracking-tight group-hover:text-fuchsia-400 transition-all truncate">
                {t.title}
              </span>
            </button>
          </div>
