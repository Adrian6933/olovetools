  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[100] pointer-events-none">
      <header className="w-full h-auto md:h-24 py-4 md:py-0 border-b border-white/10 bg-[#0a0408]/95 backdrop-blur-3xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] pointer-events-auto">
        {/* Both names have to survive 375px side by side. The brand loses its
            wordmark below `sm` (the heart still links home) and the tool name
            drops a type step, which is what keeps the row from being clipped by
            the wrapper's overflow-x-hidden instead of merely overflowing. */}
        <div className="max-w-7xl mx-auto h-full px-3 sm:px-4 md:px-12 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0">
          <div className="flex items-center justify-between w-full md:w-auto gap-3 sm:gap-4 md:gap-12 min-w-0">
            ...
            >
              ...
              <span className="text-lg sm:text-xl md:text-2xl font-black text-white tracking-tight group-hover:text-violet-400 transition-all truncate">
                {t.title}
              </span>
            </button>
          </div>
