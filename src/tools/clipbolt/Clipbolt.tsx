...
interface ClipboltProps {
  lang: Language;
  dictionary?: any;
}

const Clipbolt: React.FC<ClipboltProps> = ({ lang = 'en' }) => {
  ...
  const { t } = useTranslation(lang, 'clipbolt');

  const qualityOptions = [
    { id: 'max', label: 'Max Quality', sub: 'Original Source', icon: <Diamond className="w-4 h-4 text-blue-400" /> },
    { id: '720', label: '720p HD', sub: 'Balanced', icon: <Film className="w-4 h-4 text-twitch" /> },
  ...
  useEffect(() => {
    ...
    const storageClips = localStorage.getItem('clipbolt_shared_clips');
  ...
  const downloadAllAsZip = async () => {
    ...
    try {
      ...
      const zipName = `clipbolt_${selectedQuality}_pack_${new Date().getTime()}.zip`;
  ...
  const handleLangChange = (code: string) => {
    ...
    window.location.href = `/${code}/clipbolt`;
  };
