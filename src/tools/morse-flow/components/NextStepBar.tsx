...
import { ArrowRight, AudioLines, Braces, Binary, FileText, Loader2, QrCode } from 'lucide-react';
...
const STEPS: Step[] = [
  { slug: 'audiosnap', icon: <AudioLines className="h-4 w-4" />, key: 'nextAudio', fallback: 'Edit the audio' },
  { slug: 'binary-flow', icon: <Binary className="h-4 w-4" />, key: 'nextBinary', fallback: 'Convert to binary' },
  { slug: 'qr-bolt', icon: <QrCode className="h-4 w-4" />, key: 'nextQr', fallback: 'Make a QR' },
  { slug: 'wordflow', icon: <FileText className="h-4 w-4" />, key: 'nextWordflow', fallback: 'Count the text' },
  { slug: 'base64-bolt', icon: <Braces className="h-4 w-4" />, key: 'nextBase64', fallback: 'Encode it' },
];
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult, disabled }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'morse-flow');
    } catch {
      setPending(null);
    }
  };
      ...
      <div className="flex flex-wrap gap-2">
        {STEPS.map(step => (
          <button
            ...
            className="group flex cursor-pointer items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-xs font-bold text-slate-300 outline-none transition-all hover:border-amber-500/30 hover:bg-amber-500/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
          >
