...
import { ArrowRight, Braces, FileCode2, Fingerprint, Loader2, PenLine, Repeat } from 'lucide-react';
...
// What leaves this tool is the reviewed text (or the patch), so the next steps
// are the text tools: format it, screenshot it, write it up, hash it, convert.
const STEPS: Step[] = [
  { slug: 'json-flow', icon: <Braces className="w-4 h-4" />, key: 'nextJson', fallback: 'Format the JSON' },
  { slug: 'codecard', icon: <FileCode2 className="w-4 h-4" />, key: 'nextCodecard', fallback: 'Make a code image' },
  { slug: 'markdown-live', icon: <PenLine className="w-4 h-4" />, key: 'nextMarkdown', fallback: 'Write it up' },
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult, disabled }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'diffsnap');
    } catch {
      setPending(null);
    }
  };
    ...
    <div className="rounded-2xl border border-white/5 bg-black/30 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="hidden sm:inline text-[10px] font-medium text-slate-600 truncate">
          {t.nextStepHint || 'The revised text travels with you — no download, no re-upload'}
        </span>
      </div>
