...
export const Footer: React.FC<FooterProps> = ({ lang, t, onOpenModal }) => {
  return (
    <footer className="py-24 md:py-36 border-t border-white/5 flex flex-col items-center space-y-16 relative z-10 bg-[#04080a] w-full">
      <div className="flex flex-col items-center space-y-16 max-w-5xl px-8 text-center">
        ...
        {/* QrReader.tsx ya renderiza el bloque SEO y la FAQ dentro de <main>.
            Repetirlos aquí ponía cada párrafo de la página dos veces. */}
        ...
        <div className="flex flex-col md:flex-row flex-wrap items-center justify-center gap-y-2 md:gap-y-6 gap-x-4 md:gap-x-8 text-gray-800 font-black text-[11px] md:text-xs tracking-widest pt-12 border-t border-white/5 w-full uppercase">
          ...
          <button onClick={() => onOpenModal('privacy')} className="w-full md:w-auto py-3 md:py-0 border-none bg-transparent hover:text-cyan-400 active:bg-white/5 active:scale-95 transition-all cursor-pointer whitespace-nowrap rounded-xl text-center font-black outline-none">{legalTranslations[lang]?.nav.privacy || 'Privacy Policy'}</button>
          <button onClick={() => onOpenModal('terms')} className="w-full md:w-auto py-3 md:py-0 border-none bg-transparent hover:text-cyan-400 active:bg-white/5 active:scale-95 transition-all cursor-pointer whitespace-nowrap rounded-xl text-center font-black outline-none">{legalTranslations[lang]?.nav.terms || 'Terms of Service'}</button>
          <button onClick={() => onOpenModal('cookies')} className="w-full md:w-auto py-3 md:py-0 border-none bg-transparent hover:text-cyan-400 active:bg-white/5 active:scale-95 transition-all cursor-pointer whitespace-nowrap rounded-xl text-center font-black outline-none">{legalTranslations[lang]?.nav.cookies || 'Cookie Policy'}</button>
          <a href={`/${lang.toLowerCase()}/about`} className="w-full md:w-auto py-3 md:py-0 hover:text-cyan-400 active:bg-white/5 active:scale-95 transition-all cursor-pointer whitespace-nowrap rounded-xl text-center font-black">{legalTranslations[lang]?.nav.about || 'About'}</a>
          <button onClick={() => { navigator.clipboard.writeText(t.emailAddress || 'adrian.contact.me.69@gmail.com'); const button = document.getElementById('copy-email-footer-btn'); if (button) { const originalText = button.innerText; button.innerText = t.emailCopied || 'Copied!'; setTimeout(() => { button.innerText = originalText; }, 2000); } }} id="copy-email-footer-btn" className="w-full md:w-auto py-3 md:py-0 border-none bg-transparent hover:text-cyan-400 active:bg-white/5 active:scale-95 transition-all cursor-pointer rounded-xl font-black outline-none">{t.emailAddress || 'adrian.contact.me.69@gmail.com'}</button>
        </div>
      </div>
    </footer>
  );
