...
export const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({ currentLang, onLanguageChange }) => {
  ...
  return (
    <div className="relative w-full flex justify-center" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute left-1/2 -translate-x-1/2 md:left-auto md:translate-x-0 md:right-0 mt-12 w-48 bg-[#04080a] border border-cyan-950 rounded-lg shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="py-1">
            {LANGUAGES.map((lang) => (
              <button key={lang.code} onClick={() => { onLanguageChange(lang.code); setIsOpen(false); }} className={`w-full flex items-center px-4 py-3 text-sm hover:bg-cyan-950/30 transition-colors cursor-pointer text-left ${currentLang === lang.code ? 'bg-cyan-950/60 text-white' : 'text-gray-300'}`}>
                <span className={`font-bold mr-3 w-6 uppercase text-cyan-400`}>{lang.code}</span>
                <span>{lang.name}</span>
              </button>
            ))}
          </div>
