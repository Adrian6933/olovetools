...
import { ArrowRight, Braces, FileCode2, Fingerprint, GitCompareArrows, Loader2, Type } from 'lucide-react';
...
const STEPS: Step[] = [
  { slug: 'json-flow', icon: <Braces className="h-4 w-4" />, key: 'nextJsonFlow', fallback: 'Explore the JSON' },
  { slug: 'diffsnap', icon: <GitCompareArrows className="h-4 w-4" />, key: 'nextDiff', fallback: 'Compare it' },
  ...
  { slug: 'wordflow', icon: <Type className="h-4 w-4" />, key: 'nextWordflow', fallback: 'Count the text' },
];
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult, disabled }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'xml-json');
    } catch {
      setPending(null);
    }
  };
      ...
      <div className="flex flex-wrap gap-2">
        {STEPS.map(step => (
          <button
            ...
            className="group flex cursor-pointer items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-xs font-bold text-slate-300 outline-none transition-all hover:border-teal-500/30 hover:bg-teal-500/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
          >
