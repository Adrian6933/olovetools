      ...
      <header className="w-full h-auto md:h-24 py-4 md:py-0 border-b border-white/10 bg-[#0a0408]/95 backdrop-blur-3xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] pointer-events-auto">
        <div className="max-w-7xl mx-auto h-full px-4 md:px-12 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0">
          <div className="flex items-center justify-between w-full md:w-auto space-x-4 md:space-x-12">
            ...
            <button onClick={withScrollToTop(onReset)} {...propsAbrirEnOtraPestana} className="flex items-center space-x-2 md:space-x-3 group border-none bg-transparent outline-none transition-all hover:translate-x-1 cursor-pointer"
              title={t.resetHint || 'Start over'}
              aria-label={t.resetHint || 'Start over'}
            >
              <div className="w-8 h-8 md:w-9 md:h-9 bg-white/5 rounded-lg md:rounded-xl flex items-center justify-center border border-white/10 group-hover:bg-violet-500/20 group-hover:border-violet-500/50 transition-all">
                <Eraser className="w-5 h-5 text-violet-400" />
              </div>
              <span className="text-xl md:text-2xl font-black text-white tracking-tight group-hover:text-violet-400 transition-all">{t.title}</span>
            </button>
          </div>
