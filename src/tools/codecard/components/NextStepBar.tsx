...
interface NextStepBarProps {
  lang: string;
  t: any;
  /** Genera el PNG. Se llama en perezoso, solo al elegir un destino. */
...
// El resultado es un PNG listo para publicar, así que los siguientes pasos
// naturales son adelgazarlo, recortarlo, cambiarle el formato o marcarlo.
const STEPS: Step[] = [
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'codecard');
    } catch {
      setPending(null);
    }
  };
    ...
    <div className="rounded-2xl border border-white/5 bg-black/30 p-4 space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[10px] font-medium text-slate-600">
          {t.nextStepHint || 'Your image travels with you — no re-upload'}
        </span>
      </div>
