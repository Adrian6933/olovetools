  ...
  return new Promise((resolve, reject) => {
    ...
    script.onload = () => {
      if ((window as any).Hls) {
        ...
        reject(new Error('Hls not found on window'));
      }
    };
...
const ClipResult: React.FC<ClipResultProps> = ({ data, index, onReset, lang, dictionary }) => {
  ...
  useEffect(() => {
    ...
    if (isPlaying && videoRef.current) {
      ...
      const videoUrl = data.resolutions[0].url;

      // Apply saved volume/mute preferences
      ...
      if (videoUrl.includes('.m3u8')) {
        if (video.canPlayType('application/vnd.apple.mpegurl')) {
          ...
          loadHls().then((HlsClass) => {
            ...
            hls.on(HlsClass.Events.ERROR, (event: any, errData: any) => {
              if (errData.fatal) {
            ...
            console.error("Failed to load Hls.js, trying native fallback", err);
            video.src = videoUrl;
          });
        }
      } else {
  ...
  return (
    <div className="w-full relative group animate-fade-in">
      <div className="absolute -top-3 -left-3 z-40 bg-kick text-white font-black text-xs px-4 py-2 rounded-xl shadow-xl ring-4 ring-[#070708]">
          #{index}
      </div>
          ...
          <div className="lg:col-span-7 p-8 md:p-10 border-b lg:border-b-0 lg:border-r border-white/5">
                ...
                {isPlaying ? (
                    ...
                    <>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center bg-black/20 cursor-pointer" onClick={() => setIsPlaying(true)}>
                            <div className="w-14 h-14 bg-kick rounded-full flex items-center justify-center shadow-2xl group-hover/video:scale-110 transition-transform">
                        ...
                        <div className="absolute bottom-4 right-4 bg-black/80 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2">
                            <Clock className="w-3.5 h-3.5 text-kick" /> {Math.round(data.duration)}s
                        </div>
                    </>
                )}
             </div>
             ...
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <img src={data.broadcasterImg} alt={`Avatar en Kick de ${data.broadcaster}`} loading="lazy" decoding="async" className="w-12 h-12 rounded-full border-2 border-kick/50" />
                    <div>
             ...
             {data.resolutions.map((res, i) => {
                ...
                return (
                    <button
                        ...
                        className={`w-full group/btn relative rounded-2xl p-5 text-left flex items-center justify-between border border-white/5 transition-all overflow-hidden cursor-pointer active:scale-[0.98] ${i === 0 ? 'bg-kick/5 border-kick/20 shadow-lg shadow-kick/5 hover:bg-kick/10' : 'bg-dark-700 hover:bg-dark-600 hover:border-white/10'}`}
                    >
                        {isDownloading && (
                            <div 
                                className={`absolute inset-0 bg-kick/20 transition-all duration-300 pointer-events-none ${isCompleted ? 'bg-green-500/10' : ''}`} 
                        ...
                        <div className="relative z-10 w-full pr-4">
                            <div className="flex justify-between items-end mb-1">
                                <p className={`text-[10px] font-black uppercase tracking-widest transition-colors ${isCompleted ? 'text-green-500' : (i === 0 ? 'text-kick' : 'text-gray-500')}`}>
                        ...
                        <div className={`relative z-10 p-3 bg-white/5 rounded-xl transition-all duration-300 shrink-0 ${isDownloading ? (isCompleted ? 'bg-green-500' : 'bg-kick shadow-[0_0_15px_rgba(83,252,24,0.4)]') : 'group-hover/btn:bg-kick'}`}>
                            {isCompleted ? (
