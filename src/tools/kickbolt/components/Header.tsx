...
const Header: React.FC<HeaderProps> = ({ currentLang, onLangChange, onReset, resetHint }) => {
           ...
           <div className="relative" ref={dropdownRef}>
             <button 
               ...
               className={`
                 ...
                 ${isLangOpen ? 'border-kick ring-4 ring-kick/10' : 'hover:border-white/20 hover:bg-[#1c1c24]'}
               `}
             >
                     ...
                     {LANGUAGES.map((l) => (
                       <button
                         ...
                         className={`
                           ...
                           ${currentLang === l.code ? 'bg-kick/10' : ''}
                         `}
                       >
                         <div className="flex items-center gap-4">
                           <span className={`
                             w-9 h-9 rounded-full flex items-center justify-center text-[10px] font-black uppercase transition-all 
                             ${currentLang === l.code ? 'bg-kick text-white shadow-lg shadow-kick/20' : 'bg-white/5 text-gray-500 group-hover/item:bg-white/10 group-hover/item:text-gray-300'}
                           `}>
                             {l.code}
                           </span>
                         ...
                         {currentLang === l.code && <div className="w-2 h-2 bg-kick rounded-full font-bold text-xs" />}
                       </button>
                     ))}
                   </div>
                 </motion.div>
