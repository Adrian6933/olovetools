...
const Footer: React.FC<FooterProps> = ({ lang, onOpenLegal }) => {
  ...
  return (
    <footer className="bg-[#070708] border-t border-white/5 pt-32 pb-16 relative overflow-hidden">
      {/* Background Glows */}
      <div className="absolute top-0 left-1/4 -translate-x-1/2 w-[600px] h-[600px] bg-kick/5 blur-[160px] rounded-full pointer-events-none opacity-40"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-24">
              ...
              <div className="space-y-10">
                  <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-kick rounded-2xl flex items-center justify-center shadow-2xl">
                          <Zap className="w-7 h-7 text-white fill-current" />
                      </div>
                      <span className="text-3xl font-[900] italic uppercase text-white tracking-tight">Kickbolt</span>
                  </div>
              ...
              <div className="space-y-10">
                  <h4 className="text-white font-black uppercase text-xs flex items-center gap-3 tracking-widest italic">
                    <Search className="w-4 h-4 text-kick" /> {t.quickSearches || 'QUICK SEARCHES'}
                  </h4>
                  <ul className="space-y-5 text-sm text-gray-500 font-bold">
                      {(t.quickLinks || ['Kick to MP4', 'Download Clips', 'Bulk Downloader', 'Clip Saver', '1080p Download', 'Stream Tools']).map(link => (
                        <li key={link} className="hover:text-kick transition-colors cursor-pointer flex items-center gap-3 group">
              ...
              <div className="space-y-10">
                  <h4 className="text-white font-black uppercase text-xs flex items-center gap-3 tracking-widest italic">
                    <Tag className="w-4 h-4 text-kick" /> {t.seoKeywords || 'SEO KEYWORDS'}
                  </h4>
                  <div className="flex flex-wrap gap-2.5">
                      {(t.seoTags || [
                        'KICK DOWNLOADER', 'MP4 HD', 'BULK DOWNLOAD', 'NO LOGIN', 'FREE TOOL', '1080P', 'KICK CLIPS', 'EDITOR KIT',
                        'TIKTOK CONVERTER', 'GAMING VIDEO', 'REELS MAKER', 'STREAMING KIT', 'HIGH DEFINITION', 'DIRECT DOWNLOAD'
                      ]).map(tag => (
                          <span key={tag} className="px-4 py-2 bg-[#111114] border border-white/5 rounded-xl text-[10px] font-black text-gray-500 uppercase tracking-tighter hover:border-kick/40 hover:text-white transition-all cursor-default">
                              {tag}
                          </span>
                      ))}
                  </div>
              ...
              <div className="space-y-10">
                  <h4 className="text-white font-black uppercase text-xs flex items-center gap-3 tracking-widest italic">
                    <Layers className="w-4 h-4 text-kick" /> {t.downloadTrends || 'DOWNLOAD TRENDS'}
                  </h4>
          ...
          <div className="pt-12 border-t border-white/5 space-y-12">
              <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                  <div className="flex flex-col items-center md:items-start gap-4">
                      <p className="text-[10px] font-black text-gray-600 uppercase tracking-[0.25em]">
                          © {new Date().getFullYear()} Kickbolt • Kick Downloader
                      </p>
                      ...
                      <div className="relative group/copy">
                        <button 
                          ...
                          className="flex items-center gap-2 text-[10px] font-bold text-gray-500 hover:text-kick transition-all cursor-pointer hover:scale-105 active:scale-95 bg-white/5 px-4 py-2 rounded-lg"
                        >
                          <Mail className="w-3.5 h-3.5" />
                          adrian.contact.me.69@gmail.com
                        </button>
                        {copied && (
                          <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-kick text-white text-[10px] font-black px-3 py-1.5 rounded-lg animate-bounce shadow-lg whitespace-nowrap">
                            {t.copied}
                          </div>
                        )}
                      </div>
                   ...
                   <nav className="flex flex-col md:flex-row items-center gap-y-2 md:gap-8 w-full md:w-auto">
                       <a 
                        href={`/${lang}/privacy`}
                        className="w-full md:w-auto py-4 md:py-0 text-[11px] font-[900] text-gray-500 hover:text-kick uppercase tracking-[0.3em] transition-all cursor-pointer whitespace-nowrap active:bg-white/5 active:scale-95 rounded-2xl text-center"
                      >
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
                  ...
                  <p className="text-gray-600 text-xs font-bold tracking-tight">
                      Built by <a href={`/${lang}`} className="text-kick hover:underline">oLoveTools</a>
                  </p>
              </div>
          </div>
      </div>
