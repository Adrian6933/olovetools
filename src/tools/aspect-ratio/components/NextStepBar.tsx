...
interface NextStepBarProps {
  lang: string;
  t: any;
  /** Renders the framed result. Called lazily, only when a step is picked. */
...
// Once the frame is decided, the natural next moves are: trim it further,
// shrink it, change its container, brand it, or size it for a platform.
const STEPS: Step[] = [
  { slug: 'cropsnap', icon: <Crop className="w-4 h-4" />, key: 'nextCrop', fallback: 'Fine-tune the crop' },
  ...
  { slug: 'socialbolt', icon: <Share2 className="w-4 h-4" />, key: 'nextSocial', fallback: 'Size it for social' },
];
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'aspect-ratio');
    } catch {
      setPending(null);
    }
  };
    ...
    <div className="rounded-2xl border border-white/5 bg-black/30 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-medium text-slate-600 hidden sm:inline">
          {t.nextStepHint || 'The framed image travels with you — no re-upload'}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {STEPS.map(step => (
          <button
            ...
            className="group flex items-center gap-2 px-3.5 py-2.5 rounded-xl border border-white/10 bg-white/5 hover:bg-lime-500/10 hover:border-lime-500/30 text-slate-300 hover:text-white text-xs font-bold transition-all cursor-pointer disabled:opacity-40 disabled:cursor-wait"
          >
