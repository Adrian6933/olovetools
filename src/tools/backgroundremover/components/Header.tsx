...
export const Header: React.FC<HeaderProps> = ({ currentLang, onLanguageChange, onReset, t }) => {
  return (
    <div className="fixed top-0 left-0 right-0 z-[100] pointer-events-none">
      <header className="w-full h-auto md:h-24 py-4 md:py-0 border-b border-white/10 bg-[#08060b]/95 backdrop-blur-3xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] pointer-events-auto">
        <div className="max-w-7xl mx-auto h-full px-4 md:px-12 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-0">
          <div className="flex items-center justify-between w-full md:w-auto space-x-4 md:space-x-12">
            ...
            >
              <div className="w-9 h-9 md:w-11 md:h-11 bg-fuchsia-600 rounded-xl md:rounded-2xl flex items-center justify-center group-hover:bg-fuchsia-500 transition-all group-hover:rotate-6 group-hover:scale-110 shadow-lg shadow-fuchsia-600/40">
                <HeartIcon />
              </div>
              <div className="text-2xl md:text-3xl font-black tracking-tighter transition-all group-hover:scale-105">
                ...
                <span className="text-fuchsia-400">Tools</span>
              </div>
            </a>
            ...
            >
              <div className="w-8 h-8 md:w-9 md:h-9 bg-white/5 rounded-lg md:rounded-xl flex items-center justify-center border border-white/10 group-hover:bg-fuchsia-500/20 group-hover:border-fuchsia-500/50 transition-all">
                <ScissorsIcon />
              </div>
              {/* Below sm the icon alone carries it: "oLoveTools" plus the tool
                  name overflows a 375px viewport and gets clipped. */}
              <span className="hidden sm:inline text-xl md:text-2xl font-black text-white tracking-tight group-hover:text-fuchsia-400 transition-all">
                {t.title}
              </span>
            </button>
          </div>
