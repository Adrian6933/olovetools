...
interface NextStepBarProps {
  lang: string;
  t: any;
  /** Produces the finished cutout. Called lazily, only when a step is picked. */
...
// The cutout is a transparent PNG, so the natural next moves are: shrink it,
// crop it, change its format, brand it, or drop it into a meme.
const STEPS: Step[] = [
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'backgroundremover');
    } catch {
      setPending(null);
    }
  };
    ...
    <div className="rounded-2xl border border-white/5 bg-black/30 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-medium text-slate-600">
          {t.nextStepHint || 'Your cutout travels with you — no re-upload'}
        </span>
      </div>
