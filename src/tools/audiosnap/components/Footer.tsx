...
/**
 * Branding and legal only. The SEO copy and the FAQ used to be repeated here on
 * top of the page body and the SSR block in index.astro — three copies of the
 * same paragraphs on one URL. They now live once, in the page, as an accordion.
 */
export const Footer: React.FC<FooterProps> = ({ lang, t, onOpenModal }) => {
  const linkClass =
    'w-full md:w-auto py-3 md:py-0 border-none bg-transparent hover:text-rose-400 active:bg-white/5 active:scale-95 transition-all cursor-pointer whitespace-nowrap rounded-xl text-center font-black outline-none';

  return (
    <footer className="py-20 md:py-28 border-t border-white/5 flex flex-col items-center relative z-10 bg-[#060405] w-full">
      <div className="flex flex-col items-center space-y-12 max-w-5xl w-full px-6 md:px-8 text-center">
        <div className="text-gray-600 text-[10px] md:text-xs font-black tracking-[0.4em] md:tracking-[0.6em] uppercase opacity-40">
          {t.footerCredit || 'Part of the oLoveTools suite'}
        </div>

        <a
          href={`/${lang.toLowerCase()}/`}
          className="flex items-center space-x-4 md:space-x-6 group scale-100 md:scale-[1.35] outline-none shrink-0"
        >
