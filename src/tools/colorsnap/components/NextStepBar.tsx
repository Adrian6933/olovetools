...
interface NextStepBarProps {
  lang: string;
  t: any;
  /** Renders the palette sheet. Called lazily, only when a step is picked. */
...
// What travels is the palette sheet PNG, so the useful next moves are the ones
// that take an image: shrink it, crop it, change format, brand it, caption it.
const STEPS: Step[] = [
  ...
  { slug: 'meme-bolt', icon: <Sticker className="w-4 h-4" />, key: 'nextMeme', fallback: 'Caption it' },
];
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'colorsnap');
    } catch {
      setPending(null);
    }
  };
    ...
    <div className="rounded-2xl border border-white/5 bg-black/30 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-medium text-slate-600 hidden sm:inline">
          {t.nextStepHint || 'The palette sheet travels with you — no re-upload'}
        </span>
      </div>
