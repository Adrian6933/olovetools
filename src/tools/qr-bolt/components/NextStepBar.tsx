...
interface NextStepBarProps {
  lang: string;
  t: any;
  /** Produces the finished PNG. Called lazily, only when a step is picked. */
...
// The export is a PNG, so the natural next moves are: shrink it for the web,
// crop it into a layout, change its format, or brand it.
const STEPS: Step[] = [
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'qr-bolt');
    } catch {
      setPending(null);
    }
  };
