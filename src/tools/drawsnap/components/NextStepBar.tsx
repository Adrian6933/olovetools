...
interface NextStepBarProps {
  lang: string;
  t: any;
  /** Renders the board. Called lazily, only when a step is picked. */
...
// A finished board is a PNG: shrink it, crop it, convert it, brand it or turn
// it into a meme — all without a trip through the Downloads folder.
const STEPS: Step[] = [
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'drawsnap');
    } catch {
      setPending(null);
    }
  };
    ...
    <div className="rounded-2xl border border-white/5 bg-black/30 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-medium text-slate-600 hidden sm:inline">
          {t.nextStepHint || 'Your drawing travels with you — no re-upload'}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {STEPS.map(step => (
          <button
            ...
            className="group flex items-center gap-2 px-3.5 py-2.5 rounded-xl border border-white/10 bg-white/5 hover:bg-purple-500/10 hover:border-purple-500/30 text-slate-300 hover:text-white text-xs font-bold transition-all cursor-pointer disabled:opacity-40 disabled:cursor-wait"
          >
