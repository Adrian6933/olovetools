...
// ----------------------------------------------------------------------------
// The whole editor state of this tool is a URL string plus a handful of flags,
// so a snapshot costs a few dozen bytes: there is no reason to be clever about
// diffing. What *does* matter is not pushing one entry per keystroke — typing a
// 90-character URL would leave 90 useless steps between the user and the state
// they wanted back — so consecutive edits inside `coalesceMs` replace the top
// of the stack instead of stacking on it.
//
