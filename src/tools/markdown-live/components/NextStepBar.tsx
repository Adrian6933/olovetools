...
import { ArrowRight, AudioLines, FileCode2, Fingerprint, GitCompare, Loader2, TextQuote } from 'lucide-react';
...
// What leaves this tool is a finished document, so the next steps are the tools
// that take one: measure its prose, diff it against the last draft, read it
// aloud, frame a snippet, or fingerprint the release.
const STEPS: Step[] = [
  { slug: 'wordflow', icon: <TextQuote className="w-4 h-4" />, key: 'nextWordflow', fallback: 'Check the prose' },
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult, disabled }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'markdown-live');
    } catch {
      setPending(null);
    }
  };
    ...
    <div className="rounded-2xl border border-white/5 bg-black/30 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="hidden sm:inline text-[10px] font-medium text-slate-600">
          {t.nextStepHint || 'Your document travels with you — no re-upload'}
        </span>
      </div>
