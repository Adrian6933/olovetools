...
export const Clipy: React.FC<ClipyProps> = ({ lang = 'en' }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      ...
      {/* HEADER */}
      <header className="fixed top-0 left-0 right-0 z-50 h-24 md:h-28 flex items-center bg-[#0d0d12] border-b border-white/5 shadow-none">
        <div className="w-full h-full flex items-center justify-between px-4 md:px-12 max-w-[2200px] mx-auto gap-3 md:gap-8">
          
          <div className="flex items-center gap-3 md:gap-4 flex-shrink-0">
              <a href={`/${lang}`} className="flex items-center gap-3 group px-4 py-2 hover:bg-white/5 rounded-2xl transition-all">
                <div className="flex items-center gap-3">
                  <ArrowLeft className="w-5 h-5 text-gray-500 group-hover:text-white transition-all -translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100" />
                  ...
                  <div className="hidden lg:block font-black text-lg tracking-tighter opacity-40 group-hover:opacity-100 transition-opacity">
          ...
          <div className="flex-grow max-w-[800px] px-4 md:px-0">
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
                  ...
                  {showLangMenu && (
                      <div className="absolute right-0 top-16 w-52 bg-[#0c0c0f] border border-white/10 rounded-2xl shadow-2xl z-[60] overflow-hidden animate-in slide-in-from-top-4 duration-300">
                          {Object.keys(FLAGS).map((key) => {
                              ...
                              return (
                                <button key={l} onClick={() => { localStorage.setItem('olovetools_lang', l); window.location.href = '/' + l + '/clipy'; }} className={`w-full flex items-center gap-4 px-5 py-4 text-left hover:bg-white/5 transition-all ${lang === l ? 'text-twitch-base font-black bg-white/5' : 'text-gray-400'}`}>
                ...
                <div className="relative" ref={savedListRef}>
                    <button onClick={() => setShowSavedList(!showSavedList)} className={`w-10 h-10 md:w-14 md:h-14 rounded-xl flex items-center justify-center bg-[#1c1c24] border border-white/10 transition-all active:scale-95 ${showSavedList ? 'bg-[#2c2c36] text-twitch-base' : 'text-gray-400 hover:text-white'} ${triggerShake ? 'animate-shake' : ''}`}>
                    ...
                    {showSavedList && (
                        <div className="absolute right-0 top-16 w-80 md:w-[420px] bg-[#0c0c10] border border-white/10 rounded-3xl shadow-[0_30px_80px_rgba(0,0,0,0.8)] overflow-hidden z-[60] flex flex-col max-h-[85vh] animate-in slide-in-from-top-4 duration-500">
                            <div className="bg-[#15151b] p-6 border-b border-white/5 flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={handleUndoDelete} disabled={deletedClipsStack.length === 0} className={`p-2 rounded-xl hover:bg-white/5 transition-colors ${deletedClipsStack.length > 0 ? 'text-green-400' : 'text-gray-600'}`}><Undo className="w-4 h-4" /></button>
                                    <button onClick={handleRestoreHistory} className="text-gray-400 hover:text-white p-2 rounded-xl hover:bg-white/5"><History className="w-4 h-4" /></button>
                                    <button onClick={() => setShowSavedList(false)} className="text-gray-400 hover:text-white p-2 rounded-xl hover:bg-white/5"><X className="w-4 h-4" /></button>
                                </div>
                            </div>
                            <div className="overflow-y-auto custom-scrollbar p-5 space-y-3 flex-grow">
                                {savedClips.length === 0 ? <div className="text-center py-20 text-gray-400 font-black text-sm uppercase tracking-widest">{t('no_saved_clips')}</div> : savedClips.map(clip => (
                                    <div key={clip.id} onClick={() => handleScrollToClip(clip.id)} className="bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl p-3 flex gap-4 group transition-all cursor-pointer">
                                        ...
                                        <button onClick={(e) => handleDeleteClip(e, clip.id)} className="p-2 text-gray-500 hover:text-red-500 rounded-xl hover:bg-red-500/10"><Trash2 className="w-4 h-4" /></button>
                                    </div>
                                ))}
                            </div>
                            {savedClips.length > 0 && (
                                <div className="p-6 bg-[#15151b] border-t border-white/5 flex flex-col gap-4">
                                    <div className="grid grid-cols-2 gap-3">
                                        <button onClick={handleDownloadTxt} className="bg-white/5 py-4 rounded-2xl text-[11px] font-black border border-white/5 hover:bg-white/10 transition-all uppercase tracking-widest">{t('download_txt')}</button>
                                        <button onClick={handleExternalZip} className="bg-twitch-base/70 py-4 rounded-2xl text-[11px] font-black text-white hover:bg-twitch-base transition-all uppercase tracking-widest">{t('download_zip_web')}</button>
                                    </div>
                                    <button onClick={requestDeleteAll} className="text-[10px] text-red-500/40 font-black py-2 hover:text-red-500 transition-colors uppercase tracking-[0.2em]">{t('delete_all')}</button>
                                </div>
                            )}
                        </div>
                    )}
      ...
      {/* CONTENIDO PRINCIPAL */}
      <main className="container mx-auto px-6 pt-40 md:pt-48 flex-grow max-w-[1800px] relative z-10" key={state.mode}>
        <div className="mb-14">
            {state.mode === 'categories' && (
                <div className="mb-10 flex overflow-x-auto whitespace-nowrap gap-3 pb-4 custom-scrollbar animate-in fade-in duration-700">
                    {POPULAR_TAGS.map(tag => (
                        <button key={tag} onClick={() => handleSearch(tag)} className="px-6 py-2.5 bg-[#1a1a24] border border-white/5 rounded-full text-xs font-black text-gray-400 hover:text-twitch-base hover:border-twitch-base/30 transition-all uppercase tracking-widest flex-shrink-0">#{tag}</button>
                    ))}
                </div>
            )}
            ...
            {state.mode === 'clips' && state.activeCategory ? (
                <div className="flex flex-col gap-10">
                    <button onClick={goBackToCategories} className="flex items-center gap-3 text-gray-500 hover:text-twitch-base text-lg font-black transition-all group w-fit"><ArrowLeft className="w-6 h-6 group-hover:-translate-x-2 transition-transform" /> {t('back_categories')}</button>
                    <div className="flex items-center gap-12">
                        <div className="w-32 h-44 md:w-44 md:h-60 bg-twitch-surfaceAlt rounded-[2.5rem] overflow-hidden shadow-2xl border-2 border-white/10 ring-8 ring-twitch-base/5 animate-float">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap items-center gap-8 text-sm md:text-xl text-gray-400 font-bold">
                                <span className="text-twitch-base flex items-center gap-3"><Sparkles className="w-6 h-6" /> {t('top_clips')}</span>
                                <ChevronRight className="w-6 h-6 opacity-10" />
                                <span className="text-gray-400 bg-white/5 px-6 py-3 rounded-2xl">{t(`time_${state.timeFilter}`)}</span>
                            </div>
                        </div>
                    </div>
                </div>
      ...
      <div className="fixed bottom-4 right-4 z-40">
        <button 
          onClick={scrollToTop} 
          className={`bg-[#1a1a24] text-gray-500 p-8 rounded-[2rem] border border-white/5 transition-all hover:text-white hover:bg-twitch-base hover:shadow-[0_0_30px_rgba(145,70,255,0.3)] hover:-translate-y-2 active:scale-90 ${showScrollTop ? 'opacity-100 scale-100' : 'opacity-0 scale-50 pointer-events-none'}`}
        >
