...
import { ArrowRight, Eraser, FileImage, Loader2, Minimize2, Sticker, Type } from 'lucide-react';
import { sendToTool } from '../../../lib/handoff';

interface NextStepBarProps {
  lang: string;
  t: any;
  /** Produces the finished crop. Called lazily, only when a step is picked. */
...
// A crop is a finished image, so the natural next moves are: shrink it for the
// web, change its format, cut the subject out, brand it, or meme it.
const STEPS: Step[] = [
  ...
  { slug: 'backgroundremover', icon: <Eraser className="w-4 h-4" />, key: 'nextCutout', fallback: 'Remove the background' },
...
export const NextStepBar: React.FC<NextStepBarProps> = ({ lang, t, getResult }) => {
  ...
  const go = async (slug: string) => {
    ...
    try {
      ...
      await sendToTool(slug, lang, result.blob, result.name, 'cropsnap');
    } catch {
      setPending(null);
    }
  };
    ...
    <div className="rounded-2xl border border-white/5 bg-black/30 p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="hidden sm:block text-[10px] font-medium text-slate-600">
          {t.nextStepHint || 'Your crop travels with you — no re-upload'}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {STEPS.map(step => (
          <button
            ...
            className="group flex items-center gap-2 px-3.5 py-2.5 rounded-xl border border-white/10 bg-white/5 hover:bg-rose-500/10 hover:border-rose-500/30 text-slate-300 hover:text-white text-xs font-bold transition-all cursor-pointer disabled:opacity-40 disabled:cursor-wait"
          >
