...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, categoryLabel, buttonLabel, lang }) => {
  ...
  return (
    <a 
      ...
      className="group relative bg-[#08080a]/90 backdrop-blur-xl rounded-3xl p-8 border border-white/[0.06] hover:border-indigo-500/30 transition-all duration-500 flex flex-col h-full hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-2 overflow-hidden cursor-pointer"
    >
      ...
      <div className="flex items-start justify-between mb-8 relative z-10">
        ...
        <span className="px-4 py-1.5 bg-white/5 text-slate-300 text-xs font-semibold uppercase tracking-wider rounded-full border border-white/10 backdrop-blur-sm">
          {categoryLabel}
        </span>
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {project.tags.map((tag) => {
            ...
            return (
              <span 
                key={tag} 
                className="text-[10px] font-bold tracking-wider uppercase text-slate-400 bg-white/5 px-3 py-1.5 rounded-lg border border-white/5 transition-all duration-300 hover:bg-indigo-500/20 hover:text-indigo-300 hover:border-indigo-500/30 cursor-default"
              >
