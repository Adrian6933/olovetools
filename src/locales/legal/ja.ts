export default {
  privacy: {
    ...
    content: `
      ...
      <p>ほとんどのツール（Clipy、TwitchBolt、FormatFlow、PasteSnapなど）では、処理は完全にブラウザ内で行われます。お客様のクリップ、画像、またはフォーマットされたコードを当社のサーバーに保存することはありません。</p>
      <br/>
  ...
  terms: {
    ...
    content: `
      ...
      <p>当社のツールの一部（Clipy、TwitchBoltなど）は、サードパーティのAPI（Twitchなど）と対話します。これらのツールを使用することにより、それらのサードパーティプラットフォームのそれぞれの利用規約にも従う必要があります。当社は、これらのAPIを通じて取得されたメディアコンテンツをホストまたは所有していません。</p>
    `
  },
  cookies: {
    title: "クッキーポリシー",
  ...
  about: {
    ...
    content: `
      ...
      <h2>ツール</h2>
      <ul>
        <li><strong>Clipy & TwitchBolt:</strong> お気に入りのTwitchクリップを簡単に見つけ、整理し、ダウンロードします。</li>
