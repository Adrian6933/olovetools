export default {
  "title": "JSON-Flow",
  "description": "Utilitaire développeur interactif pour formater, valider, explorer et convertir des données JSON en CSV/XML localement.",
  "beautify": "Mettre en forme",
  "minify": "Minifier",
  "sort_keys": "Trier les clés",
  "clear": "Effacer",
  "load_mock": "Charger modèle",
  "copy": "Copier",
  "copied": "Copié !",
  "indentation": "Indentation",
  "indent_2_spaces": "2 Espaces",
  "indent_4_spaces": "4 Espaces",
  "indent_tabs": "Tabulations",
  "status_valid": "JSON Valide",
  "status_invalid": "JSON Invalide : ",
  "empty_placeholder": "Collez ou glissez votre JSON ici...",
  "drop_file_prompt": "Déposez le fichier .json ou .csv pour le charger",
  "tab_tree_viewer": "Visualiseur d'arbre",
  "tab_formatted_json": "Code formaté",
  "tab_csv_export": "Exporter en CSV",
  "tab_xml_export": "Exporter en XML",
  "search_placeholder": "Rechercher des clés ou valeurs...",
  "expand_all": "Tout développer",
  "collapse_all": "Tout réduire",
  "no_nodes": "Aucune donnée disponible dans l'arbre. Entrez un JSON valide à gauche.",
  "export_csv": "Télécharger le CSV",
  "export_xml": "Télécharger l'XML",
  "import_csv": "CSV en JSON",
  "csv_placeholder": "Collez votre texte CSV ici pour le convertir en JSON...",
  "convert_csv_btn": "Convertir CSV en JSON",
  "mock_user_profile": "Profil Utilisateur",
  "mock_product_catalog": "Catalogue Produits",
  "mock_weather_data": "Prévisions Météo",
  "seo_title": "JSON-Flow | Formateur, Validateur et Convertisseur JSON en ligne",
  "seo_description": "Validez, formatez et visualisez votre JSON en arbre interactif. Convertissez du JSON en CSV, Excel ou XML, et du CSV en JSON de manière 100% locale.",
  "seoHeroTitle": "Formatez, explorez et convertissez le JSON en toute sécurité.",
  "seoHeroText": "Ne divulguez jamais de clés d'API sensibles ou de données clients à des serveurs externes. JSON-Flow s'exécute localement dans votre navigateur.",
  "seoHeroList": [
    "Traitement 100% local",
    "Convertissez en CSV, Excel et XML instantanément",
    "Visualiseur d'arbre interactif"
  ],
  "seoBrowserSpeedTitle": "Architecture 100% client",
  "seoBrowserSpeedText": "Toutes les opérations s'exécutent localement. Vos données ne transitent jamais sur internet.",
  "seoSecondaryTitle": "L'outil JSON complet pour les développeurs web.",
  "seoKeywordsTitle": "Mots-clés",
  "seoKeywords": [
    "Formateur JSON",
    "Validateur JSON",
    "JSON en CSV",
    "JSON en XML",
    "CSV en JSON",
    "Beautifier JSON"
  ],
  "seoUseCaseTitle": "Cas d'utilisation",
  "seoUseCaseText": "Idéal pour nettoyer les payloads d'API et préparer des feuilles de calcul pour l'import.",
  "seoPrivacyTitle": "Confidentialité totale",
  "seoPrivacyText": "Aucun log serveur ni tracking, isolation client complète.",
  "faqTitle": "Questions fréquentes",
  "faq": [
    {
      "question": "Mes données sont-elles envoyées sur un serveur ?",
      "answer": "Non, tout est traité dans votre navigateur."
    },
    {
      "question": "Quelle est la taille maximale ?",
      "answer": "Dépend de votre machine, gère facilement jusqu'à 50 Mo."
    }
  ],
  "footerTagline": "Des utilitaires simples et premium pour vous aider à coder et concevoir.",
  "footerCredit": "Fait partie de la suite oLoveTools"
};
